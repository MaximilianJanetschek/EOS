...
class TestExactOptimization:
    ...
    def test_excessive_pv_generation(self):
        ...
        assert any(
            abs(charge) > 0.1 for charge in result.akku_charge
        ), "Battery should show some activity with excess PV"
    ...
    def test_very_small_battery_capacity(self):
        ...
        assert all(
            charge <= test_params["pv_akku"]["max_charge_power_w"] + tolerance
            for charge in result.akku_charge
        ), "Charging exceeds power limits"
        assert all(
            charge >= -test_params["pv_akku"]["max_charge_power_w"] - tolerance
            for charge in result.akku_charge
        ), "Discharging exceeds power limits"
    ...
    def test_negative_electricity_prices(self):
        ...
        charging_during_negative = [
            result.akku_charge[i] for i in negative_price_hours if i < len(result.akku_charge)
        ]

        # Verify that there's charging during negative price periods
        assert any(
            charge > 0 for charge in charging_during_negative
    ...
    def test_intermittent_pv_generation(self):
        ...
        high_pv_hours = [
            i
            for i in range(len(intermittent_pv))
    ...
    def test_different_prediction_horizons(self, forecast_hours):
        ...
        if forecast_hours < old_length:
            ...
            test_params["ems"]["pv_prognose_wh"] = test_params["ems"]["pv_prognose_wh"][
                :forecast_hours
            ]
            test_params["ems"]["strompreis_euro_pro_wh"] = test_params["ems"][
                "strompreis_euro_pro_wh"
            ][:forecast_hours]
            test_params["temperature_forecast"] = test_params["temperature_forecast"][
                :forecast_hours
            ]

        # If requested horizon is longer, extend with repeated values
        elif forecast_hours > old_length:
            test_params["ems"]["gesamtlast"] = (
                test_params["ems"]["gesamtlast"]
                + test_params["ems"]["gesamtlast"][: forecast_hours - old_length]
            )
            test_params["ems"]["pv_prognose_wh"] = (
                test_params["ems"]["pv_prognose_wh"]
                + test_params["ems"]["pv_prognose_wh"][: forecast_hours - old_length]
            )
            test_params["ems"]["strompreis_euro_pro_wh"] = (
                test_params["ems"]["strompreis_euro_pro_wh"]
                + test_params["ems"]["strompreis_euro_pro_wh"][: forecast_hours - old_length]
            )
            test_params["temperature_forecast"] = (
                test_params["temperature_forecast"]
                + test_params["temperature_forecast"][: forecast_hours - old_length]
            )
        ...
        assert isinstance(result, ExactSolutionResponse)
        assert (
            len(result.akku_charge) == forecast_hours
    ...
    def test_soc_constraint_enforcement(self):
        ...
        # Calculate SOC at each timestep
        capacity_wh = test_params["pv_akku"]["capacity_wh"]
        ...
        for charge in result.akku_charge:
            if charge > 0:
                charge_eff = (
                    params.pv_akku.charging_efficiency if (params.pv_akku) is not None else 1
                )
        ...
        tolerance = 0.1  # Small tolerance for floating point calculations
        assert all(
            soc >= min_soc_pct - tolerance for soc in soc_values
        ), f"Minimum SOC constraint violated. Min allowed: {min_soc_pct}, Found: {min(soc_values)}"
        assert all(
            soc <= max_soc_pct + tolerance for soc in soc_values
        ), f"Maximum SOC constraint violated. Max allowed: {max_soc_pct}, Found: {max(soc_values)}"
    ...
    def test_zero_load_scenario(self):
        ...
        test_params["ems"]["pv_prognose_wh"] = [
            2000.0 if 8 <= i < 18 else 0.0 for i in range(old_length)
        ]
    ...
    def test_seasonal_pv_variations(self):
        ...
        assert (
            summer_result.akku_charge != winter_result.akku_charge
    ...
    def test_multiple_device_coordination(self):
        ...
        assert any(abs(charge) > 0.1 for charge in result.eauto_charge), "EV should show activity"

        # Verify device constraints are respected
        assert all(
            abs(charge) <= test_params["pv_akku"]["max_charge_power_w"]
            for charge in result.akku_charge
        ), "Battery power limits should be respected"
        assert all(
            abs(charge) <= test_params["eauto"]["max_charge_power_w"]
    ...
    def test_error_handling_with_invalid_inputs(self):
        ...
        test_params["ems"]["strompreis_euro_pro_wh"] = [1e-10] * len(
            test_params["ems"]["strompreis_euro_pro_wh"]
        )
        ...
        try:
            ...
            assert isinstance(
                result, ExactSolutionResponse
            ), "Optimization should complete with very small prices"
        except Exception as e:
            # If it fails, it should be due to numerical issues, not crash
            assert (
                "numerical" in str(e).lower() or "infeasible" in str(e).lower()
    ...
    def test_performance_scaling(self):
        ...
        assert isinstance(result, ExactSolutionResponse)
        assert (
            len(result.akku_charge) == horizon
    ...
    def test_buy_sell_price_spread(self, large_difference):
        ...
        test_params["ems"]["pv_prognose_wh"] = [
            3000.0 if 8 <= i < 18 else 0.0 for i in range(old_length)
        ]
    ...
    def test_edge_case_optimization(self):
        ...
        assert all(
            charge <= max_power + tolerance for charge in result.akku_charge
        ...
        minor_violations = [
            (i, charge) for i, charge in enumerate(result.akku_charge) if abs(charge) > max_power
        ]
        if minor_violations:
